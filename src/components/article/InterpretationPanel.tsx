...
export default function InterpretationPanel({ interpretation }: InterpretationPanelProps) {
  ...
  return (
    <section className="mt-8 rounded-lg border border-fg/10 bg-fg/[0.03] p-5">
      <h2 className="mb-1 text-sm font-semibold uppercase tracking-wide text-fg/50">Interprétation IA</h2>
      ...
      {interpretation.commonPoints.length > 0 && (
        <div className="mb-3">
          <h3 className="text-xs font-medium text-fg/60">Points communs entre les sources</h3>
          <p className="text-sm text-fg/80">{interpretation.commonPoints.join(", ")}</p>
        </div>
      )}

      {interpretation.divergences.length > 0 && (
        <div className="mb-3">
          <h3 className="text-xs font-medium text-fg/60">Angles divergents</h3>
          <p className="text-sm text-fg/80">{interpretation.divergences.join(", ")}</p>
        </div>
      )}

      {sources.length > 0 && (
        <div className="mb-3">
          <h3 className="text-xs font-medium text-fg/60">Mots-clés dominants par source</h3>
          <ul className="mt-1 space-y-1 text-sm text-fg/80">
            {sources.map(([sourceName, keywords]) => (
              <li key={sourceName}>
                <span className="text-fg/50">{sourceName} :</span> {keywords.join(", ")}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div>
        <h3 className="text-xs font-medium text-fg/60">Impact potentiel</h3>
        <p className="text-sm text-fg/80">{interpretation.impactNote}</p>
      </div>
    </section>
  );
}
